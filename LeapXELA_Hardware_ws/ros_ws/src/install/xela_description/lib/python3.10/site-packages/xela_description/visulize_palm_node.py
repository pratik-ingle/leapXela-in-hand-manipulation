...
def _default_urdf_path() -> str:
    ...
    return str(share_dir / "palm.urdf")
...
class VisulizePalmPybulletNode(Node):
    def __init__(self) -> None:
        super().__init__("visulize_palm_pybullet")
...
def main() -> None:
    rclpy.init()
    node: VisulizePalmPybulletNode | None = None
    try:
        node = VisulizePalmPybulletNode()
    finally:
        if node is not None:
            node.destroy_node()
        rclpy.shutdown()
