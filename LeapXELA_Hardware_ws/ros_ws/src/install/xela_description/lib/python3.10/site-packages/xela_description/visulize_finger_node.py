...
def _default_urdf_path() -> str:
    ...
    return str(share_dir / "finger.urdf")
...
def _run_pybullet(urdf_path: str, use_gui: bool = True) -> None:
    ...
    try:
        ...
        # Some container installs miss optional pybullet_data assets; don't hard-fail on the ground plane.
        try:
...
class VisulizeFingerPybulletNode(Node):
    def __init__(self) -> None:
        super().__init__("visulize_finger_pybullet")
...
def main() -> None:
    rclpy.init()
    node: VisulizeFingerPybulletNode | None = None
    try:
        node = VisulizeFingerPybulletNode()
    finally:
        if node is not None:
            node.destroy_node()
        rclpy.shutdown()
